...
# Safe symbol rendering – fall back to ASCII when the terminal can’t handle UTF-8

_encoding = getattr(sys.stdout, “encoding”, “utf-8”) or “utf-8”
_BULLET  = “\u2022” if _encoding.lower().startswith(“utf”) else “-”
_WARN    = “\u26a0\ufe0f”  if _encoding.lower().startswith(“utf”) else “!”
_CHECK   = “\u2713”        if _encoding.lower().startswith(“utf”) else “+”
_DASH    = “\u2014”        if _encoding.lower().startswith(“utf”) else “–”

def main():
parser = argparse.ArgumentParser(
description=f”Negative Space Mapper {_DASH} Identify what’s missing, not what’s wrong”,
...
if args.format == "json":
    ...
    if not result.kernel_compliant:
        print(f"\n{_WARN}  KERNEL VIOLATION: {result.violation}")
    else:
        print(f"\n{_CHECK} Kernel compliant")
```
